...
Module Query_Module
    ...
    '========================< FOR GENERAL USE >==========================
    Public connString As String = "Data Source=BTMESSQLDEV03;Initial Catalog=LFPH_LEAN;Persist Security Info=True;User ID=mesph;Password=PHFuse;TrustServerCertificate=True"
    ...
    Sub Count_ReCertification()
        ...
        If Dbconnection.State = ConnectionState.Open Then
            ...
            Dim query As String = "SELECT COUNT(*) 
                                    ...
                                    AND AAA_Exp_Date BETWEEN GETDATE() AND DATEADD(MONTH, 2, GETDATE())"
    ...
    Sub Show_EmployeeData_Home()
        ...
        If Dbconnection.State = ConnectionState.Open Then
            ...
            Home_Form.DataGridView1.Columns("ID").AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
    ...
    Sub Add_ToMasterlist()
        ...
        Else
            ...
            Dim Initial_Val As Integer = 0

            Try
                ConOpen()
                mycommand = "INSERT INTO [AAA_Cert_Masterlist_tb] ([Employee_Number],[EmployeeName], [Department], [Line], [Station], [Test_Attempt],  
                             [Test1_Trial], [Test1_Result_Trial1], [Test1_Result_Trial2], [Test1_Result_Trial3], 
                             [Test2_Trial], [Test2_Result_Trial1], [Test2_Result_Trial2], [Test2_Result_Trial3], 
                             [Test3_Trial], [Test3_Result_Trial1], [Test3_Result_Trial2], [Test3_Result_Trial3]) 
                                    VALUES (@id, @name, @department, @line, @station, @attempt, @TT1, @Test1RT1, @Test1RT2, @Test1RT3, 
                                            @TT2, @Test2RT1, @Test2RT2, @Test2RT3, @TT3, @Test3RT1, @Test3RT2, @Test3RT3)"
                Using command As New SqlCommand(mycommand, Dbconnection)
                    ...
                    command.Parameters.AddWithValue("@attempt", Initial_Val)

                    command.Parameters.AddWithValue("@TT1", Initial_Val)
                    command.Parameters.AddWithValue("@Test1RT1", Initial_Val)
                    command.Parameters.AddWithValue("@Test1RT2", Initial_Val)
                    command.Parameters.AddWithValue("@Test1RT3", Initial_Val)

                    command.Parameters.AddWithValue("@TT2", Initial_Val)
                    command.Parameters.AddWithValue("@Test2RT1", Initial_Val)
                    command.Parameters.AddWithValue("@Test2RT2", Initial_Val)
                    command.Parameters.AddWithValue("@Test2RT3", Initial_Val)

                    command.Parameters.AddWithValue("@TT3", Initial_Val)
                    command.Parameters.AddWithValue("@Test3RT1", Initial_Val)
                    command.Parameters.AddWithValue("@Test3RT2", Initial_Val)
                    command.Parameters.AddWithValue("@Test3RT3", Initial_Val)
    ...
    Sub Delete_AlpaList()
        ...
        Using deleteCmd As New SqlCommand(deleteQuery, Dbconnection)
        ...
        Try
            ...
            Using excelConn As New OleDbConnection(excelConnStr)
                ...
                Using cmd As New OleDbCommand(query, excelConn)
                    ...
                    ' Use SqlBulkCopy to insert data into SQL Server
                    Using bulkCopy As New SqlBulkCopy(Dbconnection)
                        bulkCopy.DestinationTableName = "AAA_Cert_Alphalist_tb"

                        ' Mapping Excel Columns to SQL Server Columns
                        bulkCopy.ColumnMappings.Add("Empcode", "Employee_Number")
                        bulkCopy.ColumnMappings.Add("First Name", "First_Name")
                        bulkCopy.ColumnMappings.Add("Last Name", "Last_Name")
    ...
    Sub GoodButton_Function_Exam(imageIndex As Integer)
        ...
        Try
            ...
            Using cmd As New SqlCommand(insertQuery, Dbconnection)
    ...
    '            Dim totalCmd As New SqlCommand(totalRightQuery, Dbconnection)
    ...
    '            Using statusCmd As New SqlCommand(statusUpdateQuery, Dbconnection)
    ...
    '                Using expCmd As New SqlCommand(expDateQuery, Dbconnection)
    ...
    Public Sub UpdateResults(Exam_ID As Integer)
        Try
            ...
            For Each attempt In attemptResults.Keys
                ...
                Dim totalAttemptCmd As New SqlCommand(totalAttemptQuery, Dbconnection)
    ...
    Public Sub Get_TestTrial_Result(Exam_ID As Integer, attempt As Integer)
        Try
            ...
            Using displayCmd As New SqlCommand(displayQuery, Dbconnection)
                displayCmd.Parameters.AddWithValue("@Exam_ID", Exam_ID)
                Using displayReader As SqlDataReader = displayCmd.ExecuteReader()
    ...
    Public Sub Extract_Result()
        Try
            ...
            Dim answerAdapter As New SqlDataAdapter(answerQuery, Dbconnection)
            ...
            Dim answerKeyAdapter As New SqlDataAdapter(answerKeyQuery, Dbconnection)
    ...
    Sub Show_ReCertification_ValueStream()
        ...
        If Dbconnection.State = ConnectionState.Open Then
            ...
            Dim query As String = $"SELECT ID, Employee_Number, EmployeeName, Department, Line, Station, 
                               ...
                               FROM AAA_Cert_Masterlist_tb 
                               WHERE AAA_Exp_Date IS NOT NULL 
                                  AND AAA_Exp_Date BETWEEN GETDATE() AND DATEADD(MONTH, 2, GETDATE()) AND Department = '{dept}'
                               ORDER BY EmployeeName"
    ...
    Sub Show_All_EmpData()
        ...
        If Dbconnection.State = ConnectionState.Open Then
            ...
            'View_Form.DataGridView1.Columns("EmployeeName").Width = 300 ' Set width in pixels
            View_Form.DataGridView1.Columns("Department").HeaderText = "Department"
            View_Form.DataGridView1.Columns("Department").AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
            View_Form.DataGridView1.Columns("Line").HeaderText = "Line"
            View_Form.DataGridView1.Columns("Line").AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells
    ...
    Sub Count_Employee()
        ...
        Using command As New SqlCommand(count_query, Dbconnection)
            Dim rowCount As Integer = CInt(command.ExecuteScalar())
            'Console.WriteLine(rowCount)
    ...
    Public Sub UpdateData_ofRetaker(idValue As Integer)
        Try
            ...
            Dim query As String = "UPDATE AAA_Cert_Masterlist_tb SET " &
                                  "Test_Attempt = @attempt, Test_Status = @stat, " &
                                  "Test1_Date = @T1Date, Test1_Trial = @T1Trial1, Test1_Result_Trial1 = @T1ResTrial1, Test1_Result_Trial2 = @T1ResTrial2, Test1_Result_Trial3 = @T1ResTrial3, Test1_Judgement = @T1Judge, " &
                                  "Test2_Date = @T2Date, Test2_Trial = @T2Trial1, Test2_Result_Trial1 = @T2ResTrial1, Test2_Result_Trial2 = @T2ResTrial2, Test2_Result_Trial3 = @T2ResTrial3, Test2_Judgement = @T2Judge, " &
